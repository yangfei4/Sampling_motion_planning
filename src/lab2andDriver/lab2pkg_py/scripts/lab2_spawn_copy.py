...
if __name__ == '__main__':
    ...
    for height in range(len(block_path)):
    ...
    if not missing_block:
        # Spawn three blocks
        block_name = 'block' + str(1)
        pose = Pose(Point(block_xy_pos[starting_location][0][0], 
                        block_xy_pos[starting_location][0][1], 0*0.03), Quaternion(0, 0, 0, 0))
        spawn(block_name, open(block_paths[0], 'r').read(), 'block', pose, 'world')
