...
# UR3 home location
home = np.radians([161, -86, 105, -108, -90, 70])

# Hanoi tower location 1
Q11 = [137.5*pi/180.0, -51.7*pi/180.0, 111.7*pi/180.0, -149*pi/180.0, -90*pi/180.0, 46.4*pi/180.0]
Q12 = [137.5*pi/180.0, -58.6*pi/180.0, 110.8*pi/180.0, -141.3*pi/180.0, -90*pi/180.0, 46.4*pi/180.0]
Q13 = [137.5*pi/180.0, -64.6*pi/180.0, 108.7*pi/180.0, -133.2*pi/180.0, -90*pi/180.0, 46.4*pi/180.0]

# Hanoi tower location 2
Q21 = [161.3*pi/180.0, -56*pi/180.0, 123*pi/180.0, -157*pi/180.0, -90*pi/180.0, 70*pi/180.0]
Q22 = [161.3*pi/180.0, -63*pi/180.0, 122*pi/180.0, -148*pi/180.0, -90*pi/180.0, 70*pi/180.0]
Q23 = [161.3*pi/180.0, -70.5*pi/180.0, 118.5*pi/180.0, -136.9*pi/180.0, -90*pi/180.0, 70*pi/180.0]

# Hanoi tower location 3
Q31 = [188*pi/180.0, -51*pi/180.0, 111*pi/180.0, -150*pi/180.0, -90*pi/180.0, 97*pi/180.0]
Q32 = [188*pi/180.0, -58*pi/180.0, 110*pi/180.0, -142*pi/180.0, -90*pi/180.0, 97*pi/180.0]
Q33 = [188*pi/180.0, -64*pi/180.0, 108*pi/180.0, -134*pi/180.0, -90*pi/180.0, 97*pi/180.0]
...
Q = [ [Q11, Q12, Q13], \
      [Q21, Q22, Q23], \
      [Q31, Q32, Q33] ]
A = int(input("Enter number for start tower positions <Either 0,1,2 > "))
B = int(input("Enter number for middle tower positions <Either 0,1,2 > "))
C = int(input("Enter number for end tower positions <Either 0,1,2 > "))
...
def move_block(pub_cmd, loop_rate, start_loc, start_height, \
    ...
    global Qc
    # need to assign value to A B C by using start_loc, end_loc

    ### Hint: Use the Q array to map out your towers by location and "height".
    move_arm(pub_cmd, loop_rate, home, 4.0, 4.0)  # move home
    move_arm(pub_cmd, loop_rate, Qc[start_loc][start_height], 4.0, 4.0)
    time.sleep(1.0)
    ...
    if digital_in_0 == 0:
        ...
        sys.exit()
    move_arm(pub_cmd, loop_rate, home, 4.0, 4.0)
    move_arm(pub_cmd, loop_rate, Qc[end_loc][end_height], 4.0, 4.0)
    time.sleep(1.0)
    gripper(pub_cmd, loop_rate, suction_off)
...
def main():
    ...
    sub_position = rospy.Subscriber('ur3/gripper_input', gripper_input, gripper_callback)
    ...
    while(loop_count > 0):
        ...
        move_block(pub_command, loop_rate, 0, 1, 1, 0)
        move_block(pub_command, loop_rate, 2, 0, 1, 1)
        move_block(pub_command, loop_rate, 0, 0, 2, 0)
        move_block(pub_command, loop_rate, 1, 1, 0, 0)
        move_block(pub_command, loop_rate, 1, 0, 2, 1)
        ...
        ############## Testing function of sensors (gripper) ##############
        if digital_in_0 == 0:
            print('No block!')
